...
with st.expander("使用說明", expanded=False):
    st.markdown(
        """
        1. Excel檔的格式統一如下：**E3到N3為日期**，**ABCD依序是序號、姓名、法名、組別**
        2. 上傳一份或多份「簽到表」Excel（檔名需包含班級關鍵字：**初級 / 中級 / 高級 / 研經**，
           以及日夜關鍵字：**日 / 夜**，例如「初級夜簽到表.xlsx」）
        3. 上傳一份「課程名稱」Excel：初級／中級／高級 各一個分頁（日夜共用）
        4. 按下「產生補課通知單」
        5. 檢查預覽結果，確認無誤後下載 ZIP（內含每位學員的 Word 通知單）
        """
    )
